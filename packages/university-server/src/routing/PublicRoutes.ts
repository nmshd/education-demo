...
import { emptyApplication } from "./application";
import { Attachment, SendCustomMessageRequest } from "./Attachment";
import * as KeycloakHelper from "./keycloakHelperFunctions";
import { extractSessionId, getSocketFromCookie, getSocketFromEnmeshedAddress } from "./sessionHelper";
...
export class PublicRoutes {
  ...
  public static handleEnmeshedRequestChanged(req: express.Request, res: express.Response): any {
    if (req.headers["x-api-key"] !== config.get("connector.apiKey")) {
      return res.sendStatus(401);
    }
    ...
    const body:
      | {
          trigger: string;
          data: { request: ConnectorRequest; oldStatus: ConnectorRequestStatus; newStatus: ConnectorRequestStatus };
        }
      | undefined = req.body;
    ...
    if (request.newStatus !== ConnectorRequestStatus.Completed) {
      return;
    }

    const socket = getSocketFromEnmeshedAddress(request.request.peer);

    if (!socket) {
      console.error("Could not find socket to match enmeshed address!");
      return;
    }

    const application = parseApplicationData(request.request);

    if (!application) {
      console.error("Could not parse application data");
      return;
    }
...
function parseApplicationData(request: ConnectorRequest): Application | undefined {
  const responseData = request.response?.content.items;
  ...
  const applicationData = emptyApplication();

  console.log(JSON.stringify(data, null, 2));

  for (const responseItem of data) {
    if (responseItem["@type"] !== "ReadAttributeAcceptResponseItem") {
      continue;
    }
    const attribute = responseItem.attribute;
    ...
    switch (type) {
      case "BirthDate":
        ...
        break;
      case "BirthPlace":
        applicationData.birthPlace = {
          city: value.city,
          country: value.country
        };
        break;
      case "Nationality":
        applicationData.nationality = value.value;
        break;
      case "Sex":
        applicationData.sex = value.value;
        break;
      case "EMailAddress":
        applicationData.email = value.value;
        break;
      case "StreetAddress":
        ...
        break;
      case "PhoneNumber":
        applicationData.phoneNumber = value.value;
        break;
      default:
    }
  }
